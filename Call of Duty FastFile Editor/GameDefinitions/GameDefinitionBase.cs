...
namespace Call_of_Duty_FastFile_Editor.GameDefinitions
{
    ...
    public abstract class GameDefinitionBase : IGameDefinition
    {
        ...
        public virtual bool IsWii { get; protected set; } = false;
        ...
        public virtual RawFileNode? ParseRawFile(byte[] zoneData, int offset)
        {
            ...
            {
                Debug.WriteLine($"[{ShortName}] dataLength {dataLength} (0x{dataLength:X}) is unreasonably large at 0x{offset + 4:X}. Not a valid rawfile.");
                return null;
            }
            ...
            // Validate the filename looks like a valid rawfile
            if (!IsValidRawFileName(fileName))
            {
                Debug.WriteLine($"[{ShortName}] Invalid filename '{fileName}' at 0x{fileNameOffset:X}. Not a valid rawfile.");
                return null;
            }
        ...
        /// </summary>
        protected static bool IsValidRawFileName(string name)
        {
